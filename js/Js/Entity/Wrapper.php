...
abstract class Wrapper implements \ArrayAccess
{
	...
	{
		return $this->_traverse(function(\Js\Entity $entity) {
			$names = $entity->getMetadata(\Js\Entity::MD_FIELD_NAMES);			
			$array = array();
	...
	{
		$valid = true;
		$this->_traverse(function(\Js\Entity $entity) use (&$valid) {
			if (!$entity->isValid()) {
				$valid = false;		
			}
		});	
	...
	{
		$hasErrors = false;
		$this->_traverse(function(\Js\Entity $entity) use (&$hasErrors) {
			if ($entity->hasErrors()) {
				$hasErrors = true;
			}
		});	
